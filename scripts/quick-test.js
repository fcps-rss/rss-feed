...
// Simple test with just 2 FCPS feeds for quick testing
const TEST_FEEDS = [
  {
  ...
  {
    name: 'FCPS Employee News',
    url: 'https://public.govdelivery.com/topics/FCPS_Employees_ALL/feed.rss',
    category: 'district'
  }
];
...
const fetchRSSFeed = async (feedConfig) => {
  try {
    ...
    const response = await fetch(feedConfig.url, {
      timeout: 10000,
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; FCPS RSS Test Bot)'
      }
    });
...
const buildTestSite = async () => {
  try {
    console.log('🧪 Starting FCPS Test Build (2 feeds)...\n');
    
    const { distDir, dataDir } = ensureDistDirectory();
    
    // Fetch test RSS feeds
    const feedPromises = TEST_FEEDS.map(feedConfig => fetchRSSFeed(feedConfig));
    ...
    console.log('\n🎉 FCPS Test Build Complete!');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`📊 Test Results:`);
    console.log(`   • FCPS feeds tested: ${metadata.totalFeeds}`);
...
<head>
    ...
    <title>FCPS News Test</title>
    <link rel="stylesheet" href="css/styles.css">
    <meta name="description" content="Testing FCPS RSS feeds with ${metadata.totalFeeds} sources">
</head>
<body>
    <div class="rss-feed-container">
        <header class="rss-feed-header">
            <h1>FCPS News Test</h1>
            <p class="rss-feed-description">
                🧪 <strong>Test Mode</strong> • 
                ${metadata.totalFeeds} FCPS feeds • 
            ...
            <div class="filter-controls">
                ...
                <input type="search" id="search-input" placeholder="Search FCPS news...">
            </div>
        </header>
